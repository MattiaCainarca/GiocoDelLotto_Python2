...
def disegnaFinestra(root, fields):  # Disegna la schermata di gioco:
    ...
    img = ImageTk.PhotoImage(Image.open("logolotto.png"))
    panel = tk.Label(root, image=img, bg='white')
    panel.image = img
    panel.pack(side="top", fill="both", expand="yes")
    for field in fields:
        row = tk.Frame(root)
        fg = rgbtohex(r=0, g=72, b=157)
        lab = tk.Label(row, width=15, text=field + ": ", anchor='w', font=("Arial", 20), fg='black', bg='white')
...
def openfile():
    ...
    filename = os.path.join(dirname, 'help/manuale.html')
...
b1 = tk.Button(finestra, text='Esci', font=("Arial", 20), command=finestra.quit, borderwidth=4, relief="ridge")
b1.pack(side=tk.RIGHT, padx=7, pady=7)
b2 = tk.Button(finestra, text='Gioca', font=("Arial", 20), command=lambda: gioca(), borderwidth=4, relief="ridge")
b2.pack(side=tk.RIGHT, padx=7, pady=7)
b3 = tk.Button(finestra, text='Help', font=("Arial", 20), command=openfile, borderwidth=4, relief="ridge")
b3.pack(side=tk.LEFT, padx=7, pady=7)
finestra.mainloop()
